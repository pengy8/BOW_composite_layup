...
#    print q_read
    ...
    if Select == 3:
        ...
        p_desired = np.array([1.58, 0.02, 0.])-np.array([0.08, -1.53, 0.])      
        ...
        while (not command_claps) and voice_control.read:
            ...
            if not rospy.is_shutdown() :#and (pose is not None):
                ...
                if process == 0: # Comliance control only until steady-state 
...
#                        dq_sln = QP_bow(JT,VT,Lambda,Epsilon1,q,Epsilon2,dq_pre, Closest_Pt_L , Closest_Pt_env_L, Closest_Pt_R , Closest_Pt_env_R)    
                ...
                elif process == 2:     # Layup process. The robot holds the sheet like a damping fixture   
...
#                    dq_sln = QP_bow(JT,VT,Lambda,Epsilon1,q,Epsilon2,dq_pre, Closest_Pt_L , Closest_Pt_env_L, Closest_Pt_R , Closest_Pt_env_R)    
                ...
                elif process == 3: # The robot lets go of the sheet and stays static for the operator to remove the sheet off the grippers  
                ...
                elif process == 4: # The robot can be repositioned by the operator for regrasping
                    
                    print ("sum: ",sum(abs(q_read-q_read_pre)),sum(abs(wr_read-wr_read_pre)))
                    if (sum(abs(q_read-q_read_pre))<0.1) and (sum(abs(wr_read-wr_read_pre)))>2:
                        h_LR = p_BTR-p_BTL
                        h_LR_norm = h_LR/np.linalg.norm(h_LR)
                        h_LRv = np.cross(h_LR_norm,ez)
...
#                    command_claps = 1
        ...
        sio.savemat('Data20200901_6.mat', {'t_all':t_all,
        'dq_all':dq_all,
        'VT_all':VT_all,
...
#        print ('Rk,pk',Rk,pk)
    ...
    elif Select == 1: # Manually move the robot and regrasp the grippers test for stage 4
